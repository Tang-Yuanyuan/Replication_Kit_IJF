...
# ------------------------------ Model Training ------------------------------
def train_xgb_model_bayesian(train_x, train_y, sample_weight=None, group_type="All", n_trials=50, cv=2):
    
    def objective(trial):
        # 根据 group_type 划分参数空间（如有差异可在此调整具体数值）
        if group_type == "All":
            param = {
        ...
        # Core Change: Pass through sample weights during cross-validation.
    ...
    print(f"Performing Bayesian optimization for the [{group_type}] group classification model (weighted mode)...")
    ...
    # Core Change: Apply weights during full-data fitting.
    best_model.fit(train_x, train_y, sample_weight=sample_weight)
    
    return best_model

def train_xgb_regressor_bayesian(train_x, train_y, sample_weight=None, group_type="All", n_trials=50, cv=3):
    ...
    def objective(trial):
        ...
        # Core Change: Pass-through of Weights in Regression Model Cross-Validation
        score = cross_val_score(
            model, train_x, train_y, 
            cv=cv, 
    ...
    print(f"Performing Bayesian optimization for the [{group_type}] group regression model (weighted mode)...")
    study.optimize(objective, n_trials=n_trials)
